...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header top-0 !bg-red left-0 z-40 flex w-full items-center bg-transparent ${sticky
          ...
          <div className="relative -mx-4 flex items-center justify-between">
            ...
            <div className="flex w-full items-center justify-end px-4">
              <div>
                ...
                <nav
                  id="navbarCollapse"
                  className={`navbar absolute right-0 z-30 w-[250px] rounded border-[.5px] border-body-color/50 bg-black py-4 px-6 duration-300
                  ...
                  <ul className="block lg:flex lg:space-x-12">
                    {menuData.map((menuItem, index) => (
                      <li key={menuItem.id} className="group relative" onClick={navbarToggleHandler}>
                        <Link
                          href={menuItem.path}
                          className={`flex py-2 w-full text-base text-white group-hover:opacity-70 lg:mr-0 lg:inline-flex lg:py-6 lg:px-0`}
                        >
                          {menuItem.title}
                        </Link>
                        {/* {menuItem.path ? (
