...
const DataTable = ({ columns}: Props) => {
  ...
  const handleToggleVerified = async (id: string, verified: boolean) => {
    if (window.confirm(`Are you sure you want to ${verified ? 'unverify' : 'verify'} this Sontact Details?`)) {
      try {
        const response = await axios.put(`${URL}/contact/${id}`, { verified: !verified });
